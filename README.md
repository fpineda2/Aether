...
- **Voice visuals** — a separate ribbon across the middle of the screen that follows the artist's voice, not the beat, in three switchable styles: layered *Strands*, a twisting dotted *Particles* ribbon, and mirrored rainbow *Harmonics*. It swells as a phrase is sung, settles back to a thin thread between lines, and changes shape with how low or high the voice sits. It keeps drawing in Focus mode, and the style picker remembers your choice.
...
- **Vocal isolation is a heuristic, not stem separation.** Truly pulling a voice out of a mix takes an ML model. Instead, the analysis keeps the roughly 180 Hz–4 kHz vocal range, splits it into six log-spaced bands, and weights each frequency bin by how equal its left and right levels are, since lead vocals are almost always mixed dead center while pads, guitars and reverb spread across the stereo field. Kick and bass are centered too, but they sit below the range cut-off. Mono sources fall back to a plain vocal-range filter. On instrumental tracks (including the bundled demos) it traces the lead melody instead.
