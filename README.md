...
- **Voice visuals**: a glowing trail of the last few seconds of singing, drawn across the middle of the screen so a voice can be *seen*. Its height follows the sung pitch, so the melody visibly rises, falls, slides, holds and shimmers with vibrato. Its thickness follows how strongly each syllable is sung, its color follows the vowel (cool for "oo", warm for "ah"/"ee"), and breaths and consonants add sparkle. Three styles draw that trail: braided *Strands*, a twisting ribbon of *Particles*, and *Harmonics*, which draws the voice's actual overtones as stacked lines. A track can come with its isolated vocal stem for an exact read; without one, a live filter follows the voice in the full mix. The panel shows which is in use.
