...
- **Voice visuals**: a separate ribbon across the middle of the screen that follows the artist's voice, not the beat, in three switchable styles: layered *Strands*, a twisting dotted *Particles* ribbon, and mirrored rainbow *Harmonics*. It opens up only while someone is actually singing, settles back to a thin thread between phrases, and its ripples tighten or widen with the sung pitch. A track can come with its isolated vocal stem, which the visuals read directly for an exact result; without one, a live voice filter estimates the voice from the full mix. The panel shows which of the two is in use.
...
- **Reading the voice: exact with a stem, estimated without one.** Truly separating a voice from a finished mix takes an ML model far too heavy to run every frame in a browser. So a track can ship with its isolated vocal stem (from the artist, or split once offline with a tool like Demucs). It plays silently, kept in sync with the song, and feeds only the voice analysis. Without a stem, `lib/voiceAnalysis.js` estimates the voice in three passes. First, a mid/side split in the audio graph: a centered vocal cancels out of L−R exactly, the way karaoke vocal removers work, which drops panned instruments. Second, median-filter harmonic/percussive separation, which drops drums. Third, a harmonic-sum pitch detector with octave correction and a formant check, which only lets the visual open up for a pitched sound with a voice-like overtone structure, which drops bass lines. Tested against a synthetic mix, drums, bass and wide pads each read as silent on their own and together, while a sung melody was tracked at the right pitches. A centered, sustained lead instrument such as a synth lead or sax can still pass as a voice.
